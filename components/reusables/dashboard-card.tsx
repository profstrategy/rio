  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 h-full flex flex-col">
        <div className={`${activityWindow && 'w-full flex justify-between'}`}>
          <h3 className="text-lg font-bold mb-4" style={{ color: '#0284c7' }}>
            {title}
          </h3>
          {activityWindow && <DropdownMenu>
              {Array.from(activityWindow).map(window => (
                <DropdownMenuContent>{window}</DropdownMenuContent>
              ))}
            </DropdownMenu>}
        </div>
