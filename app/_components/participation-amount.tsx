...
export const ParticipationAmount = () => {
    ...
    return (
        <div className="w-full px-2">
            <div className="relative bg-black/60 backdrop-blur-sm border border-sky-500/20 rounded-xl lg:rounded-2xl shadow-xl hover:shadow-2xl hover:border-sky-400/40 transition-all duration-300" style={{ padding: '0 .5rem' }}>
                ...
                <div className="relative z-10 grid grid-cols-3 gap-4 lg:gap-5">
                    {participation.map((itm) => (
                        <div key={itm.id} className="flex flex-col items-center py-2 border-b border-sky-500/10 last:border-b-0">
                            <div className="text-[0.5rem] lg:text-xs font-semibold text-sky-300/80 uppercase tracking-wider mb-1 lg:mb-2">
                                {itm.text}
                            </div>
