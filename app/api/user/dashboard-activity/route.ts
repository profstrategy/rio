...
export async function GET(req: NextRequest) {
  try {
    ...
    // ✅ Fixed: Correct order and added missing comma + parameters
    const [retweets, likes, tweets, searchResults] = await Promise.allSettled([
      ...
      searchRioUserActivityPaginated(username, '24h', 10), // Added window parameter
    ])
    
    // 5. Process results
    // ✅ Fixed: Properly access nested properties from resolved promises
    ...
    // ✅ Note: userRetweets already contains retweets from getRioRetweetsPaginated
    // We use userTweets for other classifications
    const activities = {
      originalTweets: userTweets.filter((t: any) => 
        !t.referenced_tweets?.length
      ),
      retweets: userRetweets, // Use the dedicated retweets data
    ...
    await Promise.allSettled(
      activityRecords.map((activity: any) =>
        prisma.activity.upsert({
          where: { tweetId: activity.tweetId, userId:activity.userId },
          create: activity,
          update: {
